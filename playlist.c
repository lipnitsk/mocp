...
void plist_add_cue (struct plist *plist, const int pos, const char *title)
{
    if (title) 
    {
        ...
        rb_insert (plist->search_tree, (void *)(intptr_t)pos);
    }
}
...
char *plist_get_item_name (const struct plist *plist, int i)
{
    ...
    if (i < plist->num)
    {
        name = xstrdup (is_cue_track (plist, i) ? plist->items[i].title_tags : plist->items[i].file); //TODO: check this
    }
    return name;
}
...
int plist_find_del_fname (const struct plist *plist, const char *searched_name)
{
	...
	for (i = 0; i < plist->num; i++) 
    {
        char *item_name = is_cue_track (plist, i) ? plist->items[i].title_tags : plist->items[i].file;
                                       ...
                                       int *last_index)
{
	...
	for (i = *last_index; i < plist->num; i++) 
    {
        ...
        {
			*last_index = i + 1;
			return is_cue_track (plist, i) ? plist->items[i].title_tags : plist->items[i].file;
		}
	}
...
void plist_cat (struct plist *a, struct plist *b)
{
	...
	for (i = 0; i < b->num; i++) {
		...
		if (!plist_deleted(b, i) && plist_find_fname(a, is_cue_track (b, i) ? b->items[i].title_tags : b->items[i].file) == -1)
